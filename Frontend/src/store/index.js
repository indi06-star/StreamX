...
export default createStore({
  ...
  actions: {
    async signUpUser({ commit }, userData) {
      try {
        await axios.post('https://stream-x-iota.vercel.app/users', userData);
    ...
    async loginUser({ commit }, loginInfo) {
      try {
        const { data } = await axios.post('https://stream-x-iota.vercel.app/users/login', loginInfo);
    ...
    async getData({ commit }) {
      try {
        const [moviesResponse, usersResponse] = await Promise.all([
          axios.get('https://stream-x-iota.vercel.app/movies'),
          axios.get('https://stream-x-iota.vercel.app/users'),
        ]);
    ...
    async deleteMovie({ dispatch }, movie_id) {
      try {
        await axios.delete(`https://stream-x-iota.vercel.app/movies/${movie_id}`);
        dispatch('getData');
      } catch {
    ...
    async postMovie({ dispatch }, movieData) {
      try {
        await axios.post('https://stream-x-iota.vercel.app/movies', movieData);
        dispatch('getData');
      } catch {
    ...
    async patchMovie({ dispatch }, { movie_id, updatedData }) {
      try {
        await axios.patch(`https://stream-x-iota.vercel.app/movies/${movie_id}`, updatedData);
        dispatch('getData');
      } catch {
    ...
    async deleteUser({ dispatch }, user_id) {
      try {
        await axios.delete(`https://stream-x-iota.vercel.app/users/${user_id}`);
        dispatch('getData');
      } catch {
    ...
    async patchUser({ dispatch }, { user_id, updatedData }) {
      try {
        await axios.patch(`https://stream-x-iota.vercel.app/users/${user_id}`, updatedData);
        dispatch('getData');
      } catch {
    ...
    async fetchComingSoonMovies({ commit }) {
      ...
      try {
        const { data } = await axios.get('https://stream-x-iota.vercel.app/coming_soon');
    ...
    async addToCart({ commit, state }, movieId) {
      try {
        const response = await axios.post(
          'https://stream-x-iota.vercel.app/cart',
          { movie_id: movieId },
          {
            headers: {
    ...
    async fetchCart({ commit, state }) {
      try {
        const response = await axios.get('https://stream-x-iota.vercel.app/cart', {
          headers: {
            Authorization: `Bearer ${state.token}`,
          },
        });
    ...
    async deleteFromCart({ commit, state }, movieId) {
      try {
        const response = await axios.delete(`https://stream-x-iota.vercel.app/cart/${movieId}`, {
          headers: {
            Authorization: `Bearer ${state.token}`,
          },
        });
    ...
    async processPaymentController({ commit, state, getters }) {
      try {
        ...
        const response = await axios.post(
          'https://stream-x-iota.vercel.app/payments', // Endpoint for processing payments
    ...
    async fetchPaymentDetails({ commit, state }, paymentId) {
      try {
        ...
        const response = await axios.get(`https://stream-x-iota.vercel.app/payments/${paymentId}`, {
